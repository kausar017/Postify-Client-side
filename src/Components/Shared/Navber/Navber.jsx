...
const Navber = () => {
  ...
  const link = (
    <>
      <NavLink to={"/"}>
        <button className="btn btn-ghost btn-sm dark:text-white">Home</button>
      </NavLink>
      <NavLink to={"/member"}>
  ...
  return (
    <div
      className={`bg-[#A480FF] fixed w-full z-10 transition-all duration-700`}
    >
      <div className="navbar container mx-auto">
        <div className="flex-1">
          ...
          <button className="btn bg-transparent border-none shadow-md text-white backdrop-blur-md">
            {theme ? "dark" : "lignt"}
          </button>
        </div>
          ...
          <div className={`${"dropdown dropdown-end"}`}>
            {user && (
              ...
              >
                <Link
                  className={`indicator ${
                    announcements.length === 0 ? "" : "animate-bounce"
                  }`}
                >
                  <svg
                  ...
                  <span className="badge badge-sm indicator-item">
                    {announcements.length === 0 ? "0" : announcements?.length}
                  </span>
                </Link>
              </div>
            )}
          ...
          {user ? (
            <div className="dropdown dropdown-end">
              ...
              >
                ...
                {user && !isAdmin && (
                  <Link className={` ml-3`} to={"/dasbord/profile"}>
                    Dashboard
                  </Link>
                )}
                <li className="ml-3" onClick={logout}>
                  Logout
                </li>
              </ul>
            </div>
